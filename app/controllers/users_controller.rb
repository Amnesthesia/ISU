class UsersController < ApplicationController
  ...
  def show
      ...
      if user_signed_in? and current_user.id != params[:id].to_i and not current_user.is_admin?
          redirect_to root_path
      end
  end
    ...
    respond_to do |format|
      ...
      if @user.update(user_params)
        if @user.application.nil?
            format.html { redirect_to new_application_path, notice: "Yay!"}
        else
  ...
  def finish
    ...
    if request.patch? && params[:user]
        if current_user.update(user_params)
            ...
            redirect_to new_application_path, notice: 'You\'re all set!'
        else
            @show_errors = true
        end
    end
